...
class AnyNet(nn.Module):
    ...
    def forward(self, left, right):
        ...
        if self.refine_spn:
            spn_out = self.refine_spn[0](nn.functional.interpolate(left, (img_size[2]//4, img_size[3]//4), mode='bilinear',align_corners=True))
            ...
            pred_flow = nn.functional.interpolate(pred[-1], (img_size[2]//4, img_size[3]//4), mode='bilinear', align_corners=True)
            refine_flow = self.spn_layer(self.refine_spn[1](pred_flow), G1, G2, G3)
            refine_flow = self.refine_spn[2](refine_flow)
            pred.append(nn.functional.interpolate(refine_flow, (img_size[2] , img_size[3]), mode='bilinear',align_corners=True))
