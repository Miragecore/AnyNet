...
class AnyNet(nn.Module):
    ...
    def forward(self, left, right):
        ...
        for scale in range(len(feats_l)):
            if scale > 0:
                wflow = F.interploate(pred[scale-1], (feats_l[scale].size(2), feats_l[scale].size(3)),
            ...
            if scale == 0:
                ...
                disp_up = F.interploate(pred_low_res, (img_size[2], img_size[3]), mode='bilinear',align_corners=True)
                pred.append(disp_up)
            else:
        ...
        if self.refine_spn:
            spn_out = self.refine_spn[0](nn.functional.interploate(left, (img_size[2]//4, img_size[3]//4), mode='bilinear',align_corners=True))
            ...
            pred_flow = nn.functional.interploate(pred[-1], (img_size[2]//4, img_size[3]//4), mode='bilinear', align_corners=True)
            refine_flow = self.spn_layer(self.refine_spn[1](pred_flow), G1, G2, G3)
            refine_flow = self.refine_spn[2](refine_flow)
            pred.append(nn.functional.interploate(refine_flow, (img_size[2] , img_size[3]), mode='bilinear',align_corners=True))
